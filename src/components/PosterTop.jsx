    ...
    return (
        <div className="poster__top">
            ...
            <ul className={clsx("poster__list", menu && 'active')}>
                ...
                {
                    types?.map(type=>(
                        <Link key={type._id} to={!type.isMovie ?  `/events/${type._id}/${type.name.replaceAll(' ','_')}`:'/movies'} className={clsx("poster__item",type.name.toLowerCase() ===name?.replaceAll('_',' ').toLowerCase() &&"active")}>{type.name}</Link>
